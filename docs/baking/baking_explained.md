...
So **bakers** are randomly elected from the list of all nodes that have declared themselves as delegates, in proportion to the amount of Tez they have.
...
The selected baker will be able to create the next block to add to the chain and communicate it to the network. He will receive a certain number of Tez as a reward.
...
Note that an endorser may have more than one endorsement slot (over the 32). However, the endorser injects a single endorsement operation, which represents all of its endorsement slots
...
Since Tezos runs a deterministic protocol, there is no real randomness. More specifically there is randomness in the snapshot selection and baking/endorsement slots attribution. The point is that we know the result before the operation occurs (unlike in PoW where you can only estimate it).
...
The random seed for cycle `n` is a 256-bit number generated at the very end of cycle `n-1` from the previous seed and the nonces to which delegates commit during cycle `n-2`.
