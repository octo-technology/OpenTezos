...
This refactoring is meant for educational purposes: do NOT use for any other usage.
...
4. The administrator reveals this secret random large number, when calling the CloseRaffle entrypoint.
...
- everyone can try to bruteforce the hash in order to find what number yielded this hash.

This method just makes it a little harder to guess the number
...
The method is very similar to what has be done before:


1. Refactoring the storage: it must keep a hash. According to the LIGO documentation, a hash has a `bytes` hash:
```js
type storage is record [
    admin : address;
    close_date : timestamp;
...
```
2. Adding the new input in the openRaffleParameter. This type should be unit, since the administrator just has to close the raffle without any other piece of information:
```js
...
4. Refactoring the entrypoint logic. For this change, the only thing to is to save the hash into the storage:
```js
function open_raffle (const jackpot_amount : tez; const close_date : timestamp; const description : option (string); const winning_ticket_number_hash : bytes; const store : storage) : returnType is
  block {
    if Tezos.source =/= store.admin then failwith("Administrator not recognized.")
