...
Luckily, some high-level languages have been developed, to make it easier for developers to write smart contracts on the Tezos Blockchain.
...
If you are interested to learn the LIGO syntax from scratch, the best way is to start with some of the Tezos academy courses (the first 10 chapters are a good start).
...
Smart contracts generally have one or several entrypoints, but it is not mandatory.
...
The number and type of fields are defined at the record definition. It cannot be changed afterwards.
...
⚠️ Note that the user has not been changed by the function.
...
The functions called for by those actions are called entrypoints.
...
Conditional logic enables to _fork_ the control flow depending on the state.
...
It is useful when the smart contract needs to know when calling an entrypoint, if enough funds are sent...
...
The contract needs an **administrator**: that will launch a raffle session, with a **description**.
...
Several values can be held in a map, but they must be of the same type. Besides, map are not meant to keep the same number of elements.
...
Take a look at the description that is added to the storage (it is an `option`).
...
Maps are a data structure that associate values of the same type to values of the same type.
...
## Modifying the storage
The new two pieces of information will be stored in the contract storage.
...
1. Defining the type parameter. This type should be a unit, since the buyer does not get to choose the ticket id:
```js
...
2. create a ticket id. Since each participant can only buy a ticket, the size of the players set gives the new ticket id.
...
In this part, we will see how to refactor the way the winner is chosen. 
...
- the administrator knows the secret number and can tamper with the number of bought tickets in order to get the winning one
- everyone can try to bruteforce the hash in order to find what number yielded this hash. But this method just makes it a little harder to guess the number.
...
```
2. Adding the new input in the openRaffleParameter. The type bytes are added in the tuple:
```js
...
Ligo is meant for smart contract development and always yields a Michelson code.
...
LIGO syntax was designed to help developers to build smart contracts by providing them with a syntax familiar to them, with:
